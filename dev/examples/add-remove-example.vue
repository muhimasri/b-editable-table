...
<div class="table-container">
    ...
    <b-editable-table :rowUpdate="rowUpdate" bordered class="editable-table" v-model="items" :fields="fields">
...
export default {
  ...
  methods: {
      handleAdd() {
        this.rowUpdate = {edit: false, id: Date.now(), action: 'add', data: {id:5, age: 20, name: 'Peter', department: 3, dateOfBirth: '2004-05-20', isActive: false}}
        // this.items.unshift({id:5, age: 20, name: 'Peter', department: 3, dateOfBirth: '2004-05-20', isActive: false});
      },
      handleSubmit(data, update) {
        this.rowUpdate = {edit: false, id: data.id, action: update ? 'update' : 'cancel'}
      },
      handleEdit(data) {
